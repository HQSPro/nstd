...
TestGroup::TestGroup(TestKind test_kind, std::string group_name, std::string&& f): kind(test_kind), group(std::move(group_name)), file(std::move(f)){}
TestGroup::TestGroup(TestKind test_kind, std::string group_name, std::string&& f, std::vector<std::string>&& case_names , std::string&& options, std::string&& tests_code): kind(test_kind), group(group_name), cases(std::move(case_names)), file(std::move(f)), make_options(std::move(options)), code(std::move(tests_code)){}
...
const std::string& TestGroup::get_group_name() const
{
    return group;
}
...
std::unordered_map<std::string, nstd::TestGroup>& TestGroupManager::get_groups()
{
    return groups;
}
...
void TestGroupManager::add_test_group(TestGroup&& test_group)
{
    try
    {
        ...
        groups.emplace(std::make_pair(test_group.get_group_name(), std::forward<TestGroup>(test_group)));
    }
