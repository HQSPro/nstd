...
class SelfRef{
    ...
    constexpr SelfRef(nstd::in_place_type_t<D> it, Args&&... args): owner(it, std::forward<Args>(args)...), ref(*nstd::any_cast<D>(&owner)){}
    ...
    inline constexpr nstd::add_const_t<nstd::add_pointer_t<B>> operator->() const
    {
        return &ref;
    }
