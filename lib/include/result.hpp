...
public:
    ...
    inline constexpr std::add_const_t<std::add_lvalue_reference_t<OkType>> operator*() const
    {
        return value;
    }
    inline constexpr std::add_lvalue_reference_t<OkType> operator*() { return value; }
    inline constexpr std::add_const_t<std::add_pointer_t<OkType>> operator->() const
    {
        return &value;
    }
...
public:
    ...
    inline constexpr std::add_const_t<std::add_lvalue_reference_t<ErrType>> operator*() const
    {
        return value;
    }
    inline constexpr std::add_lvalue_reference_t<ErrType> operator*() { return value; }
    inline constexpr std::add_const_t<std::add_pointer_t<ErrType>> operator->() const
    {
        return &value;
    }
...
private:
    ...
    constexpr Result(ResultStatus s,
                     std::add_const_t<std::add_lvalue_reference_t<ErrType>> err_value)
        : m_status(s),
          m_result(std::in_place_type_t<Err<ErrType>>{}, std::forward<ErrType>(err_value))
    {
    }
...
public:
    constexpr Result(const Result<OkType, ErrType>& res)
        : m_status(res.m_status), m_result(res.m_result)
    {
    }
    ...
    inline constexpr bool is_reported_err() const noexcept
    {
        return ResultStatus::REPORTED_ERR == m_status;
    }
    inline constexpr bool is_unreported_err() const noexcept
    {
    ...
    inline constexpr std::add_const_t<
        std::add_lvalue_reference_t<std::variant<Ok<OkType>, Err<ErrType>>>>
    operator*() const
    {
        return m_result;
    }
    inline constexpr std::add_lvalue_reference_t<std::variant<Ok<OkType>, Err<ErrType>>> operator*()
    {
        return m_result;
    }
    inline constexpr std::add_const_t<std::add_pointer_t<std::variant<Ok<OkType>, Err<ErrType>>>>
    operator->() const
    {
        return &m_result;
    }
    inline constexpr std::add_pointer_t<std::variant<Ok<OkType>, Err<ErrType>>> operator->()
    {
        return &m_result;
    }
    inline constexpr static Result<OkType, ErrType> ok(std::remove_reference_t<OkType>&& value)
    {
    ...
    inline constexpr static Result<OkType, ErrType>
    reported_err(std::remove_reference_t<ErrType>&& value)
    {
    ...
    inline constexpr static Result<OkType, ErrType>
    unreported_err(std::remove_reference_t<ErrType>&& value)
    {
...
constexpr std::add_lvalue_reference_t<nstd::type_pack_element_t<I, OkType, ErrType, ErrType>>
get(nstd::Result<OkType, ErrType>& r)
{
    if constexpr(I == 2)
    {
        return *std::get<I - 1>(*r);
    }
    else
    {
        return *std::get<I>(*r);
...
template <std::size_t I, typename OkType, typename ErrType>
constexpr std::add_rvalue_reference_t<nstd::type_pack_element_t<I, OkType, ErrType, ErrType>>
get(nstd::Result<OkType, ErrType>&& r)
{
    if constexpr(I == 2)
    {
        return std::move(*std::get<I - 1>(std::move(*r)));
    }
    else
    {
...
constexpr std::add_const_t<
    std::add_lvalue_reference_t<nstd::type_pack_element_t<I, OkType, ErrType, ErrType>>>
...
template <std::size_t I, typename OkType, typename ErrType>
constexpr std::add_const_t<
    std::add_rvalue_reference_t<nstd::type_pack_element_t<I, OkType, ErrType, ErrType>>>
get(const nstd::Result<OkType, ErrType>&& r)
{
    if constexpr(I == 2)
    {
...
template <std::size_t I, typename OkType, typename ErrType>
constexpr std::add_pointer_t<nstd::type_pack_element_t<I, OkType, ErrType, ErrType>>
get_if(nstd::Result<OkType, ErrType>* r)
{
    if constexpr(I == 2)
    {
        return &**std::get_if<I - 1>(&**r);
    }
    else
    {
...
template <std::size_t I, typename OkType, typename ErrType>
constexpr std::add_const_t<
    std::add_pointer_t<nstd::type_pack_element_t<I, OkType, ErrType, ErrType>>>
